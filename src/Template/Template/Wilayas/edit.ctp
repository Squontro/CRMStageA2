...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $wilaya->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $wilaya->id)]
            )
        ?></li>
...
<div class="wilayas form large-9 medium-8 columns content">
    <?= $this->Form->create($wilaya) ?>
    <fieldset>
        <legend><?= __('Edit Wilaya') ?></legend>
        <?php

  ...
  controller: {
          ...
          // Iserting Servcie
                insertItem: function (item) {
                    ...
                    url: "<?= $this->Url->build(['controller'=>'LevelStudes','action'=>'addJson'])?>" ,               
                ...
                updateItem: function (item) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                        xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());},
                        url: "<?= $this->Url->build(['controller'=>'LevelStudes','action'=>'editJson'])?>" ,             
                        }).done(function (response) {  
                        console.log( "done: " + JSON.stringify(response) ); 
                        d.resolve(response);
                        }).fail(function( msg ) {
                ...
                deleteItem: function (item) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                        xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());},
                        url: "<?= $this->Url->build(['controller'=>'LevelStudes','action'=>'deleteJson'])?>" ,           
                        }).done(function (response) {  
                        console.log( "done: " + JSON.stringify(response) ); 
                        d.resolve(response);
                        }).fail(function( msg ) {
